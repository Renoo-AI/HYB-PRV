...
app.post('/level/6/login', (req, res) => {
    ...
    if (username === "' OR '1'='1" || username === '" OR "1"="1') {
        ...
        res.render('level6', { error: 'Invalid credentials' });
    }
});
...
app.get('/level/9', (req, res) => {
    if (!req.cookies.role) {
        ...
        res.render('level9', { role: 'user' });
    } else {
        const role = req.cookies.role;
        if (role === 'admin') {
            res.send('THM{cookies_can_lie}');
        } else {
            res.render('level9', { role: role });
        }
    }
});
...
app.get('/level/10', (req, res) => {
    if (!req.cookies.auth) {
        ...
        res.render('level10', { auth: Buffer.from('role=user').toString('base64') });
    } else {
        ...
        try {
            ...
            if (decoded === 'role=admin') {
                ...
                res.render('level10', { auth: authBase64 });
            }
        } catch (e) {
            res.render('level10', { auth: authBase64 });
        }
...
app.post('/level/13/register', (req, res) => {
    ...
    if (username && username.length < 5) {
         ...
         res.render('level13', { error: 'Username must be less than 5 characters to get the flag, but frontend blocks it!' });
    }
});
...
app.post('/level/14/update', (req, res) => {
    if (req.body.isAdmin === 'true') {
        ...
        res.render('level14', { error: 'Profile updated. Still not admin.' });
    }
});
...
app.get('/level/17', (req, res) => {
    ...
    if (query.includes('<script>') || query.includes('alert(')) {
         res.render('level17', { query: query, flag: 'THM{xss_is_real}' });
    } else {
         res.render('level17', { query: query, flag: null });
    }
});
