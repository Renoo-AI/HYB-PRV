...
// Routes
app.get('/', (req, res) => {
    // Pass the gameData to the index to render categories
    res.render('index', { gameData: gameData });
});

// Play/Room View (The Split THM-style interface)
app.get('/room/:id', (req, res) => {
    const id = req.params.id;
    if (!gameData[id]) {
        return res.status(404).send('Room not found');
    }
    // Render the THM-style wrapper which contains the learning content and an iframe to the level
    res.render('room', {
        levelNumber: id,
        roomData: gameData[id]
    });
});

// ==========================================
// TARGET ENVIRONMENTS (The "Labs")
// By default, we force these to isolated=true so they render cleanly inside the room iframe
// ==========================================

// Level 1: View Source Hunt
app.get('/level/1', (req, res) => {
    res.render('level1', { isolated: true });
});

// Level 2: Hidden Comment Finder
app.get('/level/2', (req, res) => {
    res.render('level2', { isolated: true });
});

// Level 3: Broken Image Path
app.get('/level/3', (req, res) => {
    res.render('level3', { isolated: true });
});
...
app.get('/level/4', (req, res) => {
    res.render('level4', { isolated: true });
});
...
app.get('/level/5', (req, res) => {
    res.render('level5', { isolated: true });
});
...
app.get('/level/6', (req, res) => {
    res.render('level6', { isolated: true, error: null });
});
...
app.post('/level/6/login', (req, res) => {
    ...
    if (username === "' OR '1'='1" || username === '" OR "1"="1') {
        ...
        res.render('level6', { isolated: true, error: 'Invalid credentials' });
    }
});
...
app.get('/level/7', (req, res) => {
    ...
    if (user === 'admin') {
        ...
        res.render('level7', { isolated: true, user: user });
    }
});
...
app.get('/level/8', (req, res) => {
    ...
    if (price === '1') {
        ...
        res.render('level8', { isolated: true, price: price });
    }
});
...
app.get('/level/9', (req, res) => {
    if (!req.cookies.role) {
        ...
        res.render('level9', { isolated: true, role: 'user' });
    } else {
        const role = req.cookies.role;
        if (role === 'admin') {
            res.send('THM{cookies_can_lie}');
        } else {
            res.render('level9', { isolated: true, role: role });
        }
    }
});
...
app.get('/level/10', (req, res) => {
    if (!req.cookies.auth) {
        ...
        res.render('level10', { isolated: true, auth: Buffer.from('role=user').toString('base64') });
    } else {
        ...
        try {
            ...
            if (decoded === 'role=admin') {
                ...
                res.render('level10', { isolated: true, auth: authBase64 });
            }
        } catch (e) {
            res.render('level10', { isolated: true, auth: authBase64 });
        }
...
app.get('/level/13', (req, res) => {
    res.render('level13', { isolated: true, error: null });
});
app.post('/level/13/register', (req, res) => {
    ...
    if (username && username.length < 5) {
         ...
         res.render('level13', { isolated: true, error: 'Username must be less than 5 characters to get the flag, but frontend blocks it!' });
    }
});

// Level 14: Hidden Input Field
app.get('/level/14', (req, res) => {
    res.render('level14', { isolated: true });
});
app.post('/level/14/update', (req, res) => {
    if (req.body.isAdmin === 'true') {
        res.send('THM{hidden_is_not_safe}');
    } else {
        res.render('level14', { isolated: true, error: 'Profile updated. Still not admin.' });
    }
});
...
app.get('/level/15', (req, res) => {
    ...
    if (id === '2') {
        ...
        res.render('level15', { isolated: true, id: id });
    }
});

// Level 16: File Name Guessing
app.get('/level/16', (req, res) => {
    res.render('level16', { isolated: true });
});
...
app.get('/level/17', (req, res) => {
    ...
    if (query.includes('<script>') || query.includes('alert(')) {
         res.render('level17', { isolated: true, query: query, flag: 'THM{xss_is_real}' });
    } else {
         res.render('level17', { isolated: true, query: query, flag: null });
    }
});
...
app.get('/level/18', (req, res) => {
    ...
    res.render('level18', { isolated: true, message: storedMessage, flag: flag });
});
...
app.get('/level/19', (req, res) => {
    ...
    if (page === '../flag.txt' || page.includes('..')) {
        ...
        res.render('level19', { isolated: true, page: page });
    }
});
...
app.get('/level/20', (req, res) => {
    ...
    res.render('level20', { isolated: true });
});
